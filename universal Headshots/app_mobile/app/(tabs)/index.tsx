...
export default function Dashboard() {
  ...
  const onTouchStart = (e: any) => {
    if (step > 1 && e.nativeEvent.pageX < 100) {
      touchStart.current = { x: e.nativeEvent.pageX, y: e.nativeEvent.pageY };
    }
  };

  const onTouchEnd = (e: any) => {
    if (step > 1 && touchStart.current.x < 100) {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      const interval = setInterval(async () => {
        try {
          const res = await fetch(`${Constants.ENDPOINTS.STATUS}/${result.batchId}`);
          const status = await res.json();
          if (status.progress !== undefined) setProgress(status.progress);
          if (status.status === 'COMPLETED') {
            clearInterval(interval);
            setStep(4);
            // Send push notification when headshots are ready
            const imageCount = status.imageCount || 20;
            notifyHeadshotsComplete(imageCount);
          } else if (status.status === 'FAILED') {
            clearInterval(interval);
            notifyHeadshotsFailed(status.error || 'Generation failed');
          }
        } catch (e) {}
      }, 3000);
    } catch (error: any) {
    ...
    <SafeAreaView className="flex-1 bg-[#0A0A0F]">
      {step > 1 && (
        <View 
          ...
          style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 100, zIndex: 999999, backgroundColor: 'transparent' }} 
        />
      )}
      ...
      <View className="flex-1">
        <View className="px-6 py-4 flex-row items-center justify-between">
          ...
          <View className="bg-violet-600/20 px-3 py-1 rounded-full border border-violet-500/30">
            <Text className="text-violet-400 text-[10px] font-black uppercase tracking-widest">Step {step}/4</Text>
          </View>
        </View>
        ...
        <View className="px-6 mt-4 flex-1">
          {step === 1 && (
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
              <View className="space-y-8">
                <View>
                  <Text className="text-white text-3xl font-black italic tracking-tighter uppercase leading-tight">Studio Workspace</Text>
                  <Text className="text-violet-400 text-[10px] font-black uppercase tracking-widest mt-2">Phase 1: Input Assets</Text>
                </View>
                  ...
                  <View className="flex-row flex-wrap gap-2">
                    {images.map((uri, idx) => (
                      <View key={idx} className="w-[23%] aspect-square rounded-2xl bg-white/5 overflow-hidden border border-white/10">
                        ...
                        <TouchableOpacity onPress={() => removeImage(idx)} className="absolute top-2 right-2 bg-black/80 rounded-full p-1 border border-white/20">
                          <Icon component={X} size={10} color="white" />
                        </TouchableOpacity>
                      </View>
                    ))}
                    {images.length < 8 && (
                      <TouchableOpacity onPress={pickImage} className="w-[23%] aspect-square rounded-2xl bg-white/5 border border-dashed border-violet-500/40 items-center justify-center">
                        <Icon component={Camera} size={24} color="#8b5cf6" />
                      </TouchableOpacity>
                    )}
                  </View>
                ) : (
                  <TouchableOpacity onPress={pickImage} className="bg-white/5 border-2 border-dashed border-violet-500/20 rounded-[40px] p-12 items-center justify-center">
                ...
                {images.length >= 4 && (
                  <TouchableOpacity onPress={() => setStep(2)} className="bg-violet-600 h-16 rounded-[24px] items-center justify-center shadow-xl shadow-violet-600/40">
                    <Text className="text-white font-black text-lg italic tracking-tighter uppercase">Continue (Select Style)</Text>
                  </TouchableOpacity>
                )}
              </View>
            </ScrollView>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 150 }}>
                <View className="space-y-8">
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                    {((STYLES[gender as keyof typeof STYLES] as any)?.[country] || (STYLES[gender as keyof typeof STYLES] as any)?.['GLOBAL'] || []).map((s: any) => (
                      ...
                      >
                        ...
                        <Image 
                          ...
                          onLoad={() => console.log('[Dashboard] Style Image Loaded:', s.id)}
                          onError={() => console.error('[Dashboard] Style Image FAILED:', s.id, s.preview)}
                        />
                    ...
                    {/* Diagnostic view if no styles are mapped */}
                  ...
                  {selectedStyle && (
                    <TouchableOpacity 
                      ...
                      disabled={uploading}
                      className={`bg-violet-600 h-18 py-5 rounded-[24px] items-center justify-center shadow-2xl shadow-violet-600/40 ${uploading ? 'opacity-50' : ''}`}
                    >
          ...
          {step === 3 && (
            <View className="flex-1 items-center justify-center p-8">
              <Text className="text-white text-3xl font-black italic tracking-tighter uppercase mb-2">Processing...</Text>
              <Text className="text-violet-400 text-5xl font-black mt-4">{progress}%</Text>
            </View>
          )}

          {step === 4 && (
            <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>
              <Text className="text-white text-3xl font-black italic tracking-tight uppercase mb-8">Portfolio</Text>
              <View className="flex-row flex-wrap justify-between gap-y-3">
                {[1, 2, 3, 4].map(i => (
                  <View key={i} className="w-[48%] aspect-square bg-white/5 rounded-2xl border border-white/10" />
                ))}
              </View>
              <TouchableOpacity onPress={() => setStep(1)} className="mt-10 bg-white/5 py-4 rounded-3xl items-center">
                <Text className="text-gray-500 font-bold">START NEW SESSION</Text>
              </TouchableOpacity>
            </ScrollView>
          )}
        </View>
